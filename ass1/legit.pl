    ...
    if( $i == 0 ){
        ...
        for my $file1 (@files1){
            ...
            for my $file2 (@files2){
                ...
                if((compare(".legit/index/$file1",$file2) != 0)){
                    $count_file2++;
        ...
        elsif((($count_file2 == $k) !=0) and $count_file1 == 0 ){
            mkdir ".legit/$i",0755 or warn "can not make fred directory : $!\n";
            mkdir ".legit/$i/log",0755 or warn "can not make fred directory : $!\n";
            open(FH , '>',".legit/$i/log/log") or die $!;
            print FH $_[0];
...
sub remove(){
    ...
    for my $check_file (@check){
        ...
        my $index_file = ".legit/index/$check_file";
        opendir (DIR,'.legit/')or die "$!";
        my @list_dir = grep{!/^\./} readdir(DIR)or die "$!";
        my $i = scalar @list_dir -3;
        my $commited_file = ".legit/$i/$check_file";
...
sub set_status(){
    opendir (DIR,'.legit/index')or die "$!";
    my @all_files = grep{!/^\./} readdir(DIR)or die "$!";
...
sub status(){
    my @all_files=glob("*");
    @all_files = grep{!/regit.pl/} @all_files;
    
    opendir (DIR,'.legit/deleted')or die "$!";
    my @delete_files = grep{!/^\./} readdir(DIR)or die "$!";
